...
class Level():
    ...
    def next_level(self): 
        if self.level < self.max_level:
            ...
            return True  # Indicate that the level advanced
        else:
            return False
...
while run:
    ...
    if game_active:
        ...
        if gameScore.score >= 50:
            if level.next_level():
                gameScore.score = 0  # Reset score for the new level
                ball.update_speed(level.level)  # Increase ball speed
            else:
                # End the game when max level is reached
                game_active = False
